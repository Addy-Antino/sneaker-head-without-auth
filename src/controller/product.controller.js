...
exports.createProduct = catchAsync(async (req, res, next) => {
  ...
  const product = await Productservice.createProduct(body, image)
  res.status(201).json({
    success: true,
    product
  })
...
//*For getting all the pages
exports.getTotalnopages = catchAsync(async (req, res, next) => {
  const totalNoPages = await Productservice.getTotalnoofPages();
  res.status(200).json({
    success: true,
    totalNoPages,
  })
